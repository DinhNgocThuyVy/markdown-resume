...
pandoc -s --self-contained --resource-path=.:styles/"${STYLE}" -t html $styles "$sourcefile" -o "docs/$source_base.$format" --pdf-engine=wkhtmltopdf --pdf-engine-opt=--enable-local-file-access